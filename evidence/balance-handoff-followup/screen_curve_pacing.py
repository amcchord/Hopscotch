...
base=replace(base,absolute_capture_trim=True)
plant=s.load_fitted_params(Path('evidence/balance-review/model-fit-speed.json'))
results=[]
for curve,floor in itertools.product((1.9,3.,4.),(.3,.7,1.)):
 cfg=replace(base,sp_curve=((0.,84.),(.5,84.-curve/2),(1.,84.-curve)),ramp_vel_gate_floor=floor)
 cases=[]
...
Path('evidence/balance-handoff-followup/curve-pacing-screening.json').write_text(json.dumps(results,indent=2)+'\n')
