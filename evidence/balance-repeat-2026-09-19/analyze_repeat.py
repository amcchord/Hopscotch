"""Compare a repeat stand-up with the first successful early-recovery trial."""
import sys,json
...
if len(sys.argv)!=2:raise SystemExit('Usage: analyze_repeat.py NEW_LOG.csv')
files=[('September 14: first success','bal_20260914_233101_early-recovery-first-success.csv'),('September 19: reported success',Path(sys.argv[1]).name)]
...
for j,(label,name) in enumerate(files):
 ...
 if len(settled) and t[-1]>t[settled[0]]+.5:
 ...
 for ax in axes[:,j]:
  ...
  ax.set_xlim(0,16);ax.grid(alpha=.2);ax.legend(fontsize=7,loc='best')
...
fig.suptitle('Repeat successful stand-up: compare onset, recovery and settling\nBoth attempts reported successful. Equal vertical scales; travel remains relative to engagement.',fontsize=13)
fig.savefig(ROOT/'evidence/balance-repeat-2026-09-19/comparison.png',dpi=160)
(ROOT/'evidence/balance-repeat-2026-09-19/metrics.json').write_text(json.dumps(metrics,indent=2)+'\n')
