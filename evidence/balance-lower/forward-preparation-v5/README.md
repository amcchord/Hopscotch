...
limits, 2 missed catches); 7 are other missed catches, and 1 is asymmetric arm
...
260 for the installed policy under that paired model. It let the body outrun
...
The plant speed ceiling is 4 rad/s to permit the new preparation motor command;
actual per-phase limits still come from each compiled policy. The old policy
therefore retains its 1.5 rad/s catch and 0.3 rad/s descent cap. The preparation
...
(261 completions here versus 260 in the previous v4 screen); compare only the
...
It is about 0.33 m/s using the assumed 55 mm wheel radius. Wheels stop at 3 rad/s²
after support. Sampled body-rate 65 deg/s, feedback/owner 100 ms, load requirements,
