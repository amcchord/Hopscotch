#!/usr/bin/env python3
"""Recorded v3 impact/rebound; pinned policy replay, not a counterfactual trial."""
import csv
import ctypes as C
import hashlib
import json
...
LOG = ROOT / 'telemetry_logs/bal_20260920_forward_catch_v3_wifi.csv'
...
def analyze():
    ...
    summary = dict(log=str(LOG.relative_to(ROOT)), samples=len(rows),
                   ...
                   operator='After CH11 stopped, arms supported the still-upright robot. One CH12 press brought it flat. User requests automatic arm reversal at floor contact and final Forward pose.',
...
def replay(rows, active, summary):
    out = ROOT / 'output/lowering-v3-trial-replay'
    out.mkdir(parents=True, exist_ok=True)
    for target, source in (('balance_lower.h','src/balance_lower.h'), ('bridge.cpp','scripts/lowering_bridge.cpp')):
        (out / target).write_bytes(subprocess.check_output(['git','show','8449ddb:'+source], cwd=ROOT))
    ...
    try:
        ...
        summary['replay']=dict(source='8449ddb (same policy as installed 8449ddb)',
...
def plot(rows, active, summary):
    ...
    fig.suptitle('Physical v3 trial: early moving-arm contact and backward rebound\nRecorded telemetry · body still near upright at impact')
