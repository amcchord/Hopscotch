...
variants={'baseline':base, 'gate15':replace(base,ramp_gate_err=1.5),'gate25':replace(base,ramp_gate_err=2.5),'gate4':replace(base,ramp_gate_err=4.),'smooth':replace(base,smooth_return=True),'smooth_fast':replace(base,smooth_return=True,arm_return_speed=2.3),'smooth_gate25':replace(base,smooth_return=True,ramp_gate_err=2.5),'smooth_fast_gate25':replace(base,smooth_return=True,arm_return_speed=2.3,ramp_gate_err=2.5)}
results=[]
...
Path('evidence/balance-standup-fix/screening2.json').write_text(json.dumps(results,indent=2)+'\n')
