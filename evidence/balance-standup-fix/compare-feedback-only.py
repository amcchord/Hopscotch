...
new=s.current_firmware_config()
plant=s.load_fitted_params(Path('evidence/balance-review/model-fit-speed.json'))
results=[]
for name,cfg in [('flashed_before',base),('candidate_no_arm_change',replace(new,arm_return_acceleration=0.))]:
    cases=[]
    for af in (.7,1.,1.4):
      for bf in (.66,1.,1.33):
...
(out/'feedback-only-simulation.json').write_text(json.dumps(results,indent=2)+'\n')
