...
base=s.current_firmware_config(); (out/'candidate-config.json').write_text(json.dumps(asdict(base),indent=2)+'\n')
plant=s.load_fitted_params(Path('evidence/balance-review/model-fit-speed.json'))
results=[]
for enabled in (False,True):
 cfg=replace(base,startup_recovery=enabled)
 cases=[]
 ...
 summary=dict(recovery=enabled,cases=len(cases),early=sum(c['early'] for c in cases),late=sum(c['late'] for c in cases),no_ramp=sum(c['ramp'] is None for c in cases),median_peak=statistics.median(c['peak'] for c in cases))
 print(summary,flush=True);results.append(dict(summary=summary,cases=cases))
(out/'model-comparison.json').write_text(json.dumps(results,indent=2)+'\n')
