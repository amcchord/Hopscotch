...
class Pilot:
 def __init__(self,params,profile,capture=(False,1,2.5,8,60),brake=(0,0,1,4,.08)):
  self.governed_motion=True;self.ptr=lib.pilot_create();self.profile=profile
  if params is not None:lib.pilot_params(self.ptr,(ct.c_float*9)(*params))
  lib.pilot_stop_params(self.ptr,*capture)
  lib.pilot_brake_params(self.ptr,*brake)
  self.drive_active=False;self.moving=False;self.velocity=0.;self.arm=0.;self.rows=[];self.emergencies=0
 def __del__(self):lib.pilot_destroy(self.ptr)
 ...
 def velocity_correction(self,error,low,high,knee,ramp):return lib.pilot_correction(self.ptr,error,low,high,knee,ramp)
 def drive_step_raw(self,error,raw,old,speed,pd,previous,dt,limit):
  was_active=self.drive_active
  out=(ct.c_float*2)();lib.pilot_drive(self.ptr,error,raw,old,speed,pd,previous,dt,limit,out)
 ...
 def arm_velocity_error(self,speed,target):return lib.pilot_arm_error(speed,target)
 def arm_demand(self,demand):return lib.pilot_arm_demand(self.ptr,demand)
 def arm_emergency(self,moving,cmd,velerr,err,rate,dt,limit,old):
  result=bool(lib.pilot_emergency(self.ptr,moving,cmd,velerr,err,rate,dt,limit,old));self.emergencies+=int(moving and result);return result
 def arm_command(self):return self.arm
 def learning_allowed(self,error):return bool(lib.pilot_learning(self.ptr,error))
...
def run(params=None,profile='small',p=plant,duration=36,pushes=None,seed=1,capture=(False,1,2.5,8,60),brake=(0,0,1,4,.08)):
 pilot=Pilot(params,profiles[profile] if isinstance(profile,str) else profile,capture,brake)
