...
for A,B,wn,z,delay in itertools.product((8,25,45),(4,7,13,16),(40,60),(.35,.6),(0,.01)):
 ...
 for profile in ('push','replay'):
  ...
  pair={}
  for label,dec in [('v4',8),('ramp12',12),('ramp16',16),('ramp20',20)]:
   r,pp=run([8.8,1.5,3,.006,6,dec,.3,.12,1],fn,p,duration=58 if profile=='replay' else 36,pushes=pushes,brake=(0,0,1,4,.08))
...
(OUT/'ramp-disturbance.json').write_text(json.dumps(dict(summary=summary,cases=rows),indent=2)+'\n')
