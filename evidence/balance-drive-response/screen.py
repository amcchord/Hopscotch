...
subprocess.run(['clang++','-std=c++17','-shared','-fPIC','-O2','-Isrc','-Itests/stubs',
                str(OUT/'pilot_bridge.cpp'),'-o','output/drive_response_bridge.dylib'],cwd=ROOT,check=True)
lib=ct.CDLL(str(ROOT/'output/drive_response_bridge.dylib'))
...
lib.pilot_correction.argtypes=[ct.c_void_p]+[ct.c_float]*4+[ct.c_int]
lib.pilot_correction.restype=ct.c_float

class Pilot:
    def __init__(self, profile):
        self.ptr=lib.pilot_create();self.profile=profile;self.moving=False;self.velocity=0.;self.rows=[]
    def velocity_correction(self,error,low,high,knee,ramp):
        return lib.pilot_correction(self.ptr,error,low,high,knee,ramp)
    def __del__(self):
...
print('Neutral sticks: all 324 complete neutral trajectories identical to installed v1.',flush=True)
...
for af,bf,tau,offset in itertools.product((.7,1.,1.4),(.7,1.,1.4),(.01,.03,.06),(-.4,.4)):
    ...
    for label,profile in [('forward_stop_reverse_stop',drive),('input_loss',dropout)]:
        ...
        if af==bf==1 and tau==.03 and offset==.4:
            (OUT/f'{label}-nominal.json').write_text(json.dumps(dict(t=b.t[::5],velocity=b.wheel_vel[::5],
                drift=b.drift[::5],roll=b.roll[::5],target=b.target_vel[::5],pilot=last.rows[::5]),indent=2)+'\n')
