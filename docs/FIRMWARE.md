...
Installed identity and unresolved work live in [current state](progress/CURRENT.md).
Use [the project map](../WORKBOOK.md) to start a development task.

![Robot Diagram](RobotDiagram.png)
...
[Setup, API, safety design and recovery](WIFI_OTA.md).
...
[September 19 radio audit](RADIO_TELEMETRY.md); later radio adjustments and
...
The [CH6 fast tip-up](archive/2026-09/FAST_TIP_UP_2026-09.md) targets roughly
...
- Local `src/network_secrets.h` configured as described in the [operating guide](WIFI_OTA.md#local-configuration-and-access)
...
Use the [single-command frozen-package procedure](WIFI_OTA.md#update-the-firmware).
...
Motor power may remain on while disabled. Transmitter-on OTA has been verified; transfer speed remains an open
investigation. See the current OTA guide for supported upload profiles.
...
calibration and the saved run. USB recovery is [slot-aware](WIFI_OTA.md#recovery);
