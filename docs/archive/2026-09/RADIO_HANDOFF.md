...
**Current firmware workflow:** the Wi-Fi/OTA task installed the combined robot firmware, including the structured radio payload, on September 19. See [current robot state](../../progress/CURRENT.md). Use [OTA for robot updates and Wi-Fi for full log downloads](../../WIFI_OTA.md); Lua/handset storage updates remain separate. The actual GX12 screen/RF rendering remains unverified.

## Historical radio-task handoff

The installation record below describes that task's own completion before the
combined firmware deployment. Its then-pending integration is now complete.
