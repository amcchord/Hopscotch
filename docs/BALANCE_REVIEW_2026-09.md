...
| Motor setup | Check acceleration-write transmission and verify current-limit readback; request motor stop on setup failure. The original September acceleration-readback requirement was invalid for RS05 and caused startup refusal; see the [correction](BALANCE_START_FIX_2026-09.md). This does not add readback verification for every CAN command. |
